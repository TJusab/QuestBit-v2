...
const CustomCalendar = () => {
    ...
    const renderCustomHeader = (date) => {
        ...
        return (
            <View style={styles.header}>
                <Text className="font-zcool text-xl py-5 text-navy" style={styles.monthText}>{monthName}</Text>
            </View>
        );
    };
  ...
  return (
    <View>
      ...
      />
      <ScrollView contentContainerStyle={styles.questbitsContainer}>
          {selectedQuestbits.map((questbit, index) => (
              <Text key={index} style={styles.questbitTitle}>{questbit.title}</Text>
          ))}
      </ScrollView>
    </View>
  );
